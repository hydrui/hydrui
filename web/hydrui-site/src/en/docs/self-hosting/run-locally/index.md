...
| `-llm-url`             | `HYDRUI_LLM_URL`          | &mdash;   | OpenAI-compatible language model provider URL, excluding the `/v1` suffix (server mode only)                                                                           |
| `-llm-secure`          | `HYDRUI_LLM_SECURE`       | `true`    | Enable validating the TLS certificate of the language model provider                                                                                                   |
| `-llm-api-key`         | `HYDRUI_LLM_API_KEY`      | &mdash;   | Language model provider API key                                                                                                                                        |
| `-llm-api-key-file`    | &mdash;                   | &mdash;   | Path to a file containing the language model provider API key                                                                                                          |
| `-llm-model`           | `HYDRUI_LLM_MODEL`        | &mdash;   | Model used for server-proxied language model requests                                                                                                                  |
| `-llm-name`            | `HYDRUI_LLM_NAME`         | &mdash;   | Display name for the server-configured language model provider                                                                                                         |
